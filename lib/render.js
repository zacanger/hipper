...
module.exports = (doc, rc) => {
  ...
  function render (line, x, y) {
    ...
    var
      length = rc.columns - +rc.margin
    , pos    = Math.max(0, doc.column)
    , start  = doc.row+1 === y ? Math.max(doc.column + rc.rightMargin - length, 0) : 0
    ...
    if ((rc.margin + line.length) > (rc.columns || 80)) {
     // l = line.length
      ...
      // to do: add ellipses to end of line if off-screen
    }
  ...
  function redraw () {
    ...
    let already = [] // forget everything recently drawn
    c.erase('screen')
  ...
  function scrollUpLine () { // delete line from bottom
  ...
  function deleteLastLine () {
    c.position(1, R.header + height - offset).delete('line')
  ...
  function cursor (line, x, y) { // cursor has moved
  ...
  function scroll (line, x, y) { // where is the off-by-one error on scrolling down?
    let target = offset
